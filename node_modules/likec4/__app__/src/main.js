...
function Fallback$1({ error, resetErrorBoundary }) {
  ...
  return isNotFound(error) ? /* @__PURE__ */ jsx(Container, { my: "md", children: /* @__PURE__ */ jsxs(Alert, { variant: "light", color: "orange", children: [
    ...
    /* @__PURE__ */ jsxs(Group, { mt: "lg", children: [
      ...
      /* @__PURE__ */ jsx(Button, { onClick: () => router.navigate({ to: "/" }), color: "red", size: "xs", children: "Go to overview" })
    ] })
  ] }) });
}
const content = css({
...
const Route$k = createFileRoute("/_single")({
  staleTime: 1 / 0,
  loader: async ({ context }) => {
    const projectId = context.projectId, { $likec4data, $likec4model } = await loadModel(projectId);
    return {
      $likec4data,
      $likec4model,
      projectId
...
function RouteComponent$2() {
  const { $likec4data, $likec4model, projectId } = Route$k.useLoaderData();
  return /* @__PURE__ */ jsx("div", { className: cssViewOutlet, children: /* @__PURE__ */ jsx(ErrorBoundary, { FallbackComponent: Fallback$1, children: /* @__PURE__ */ jsx(LikeC4IconRendererContext, { projectId, children: /* @__PURE__ */ jsx(
    LikeC4ModelContext,
    {
...
const previewBg = css({
  ...
  inset: "0",
  zIndex: 5
}), Route$i = createFileRoute("/_single/single-index")({
...
function RouteComponent() {
  const { $likec4data, $likec4model, projectId } = Route$h.useLoaderData();
  return /* @__PURE__ */ jsx("div", { className: cssViewOutlet, children: /* @__PURE__ */ jsx(ErrorBoundary, { FallbackComponent: Fallback$1, children: /* @__PURE__ */ jsx(LikeC4IconRendererContext, { projectId, children: /* @__PURE__ */ jsx(LikeC4ModelContext, { likec4data: $likec4data, likec4model: $likec4model, children: /* @__PURE__ */ jsx(Outlet, {}) }) }) }) });
}
const Route$g = createFileRoute("/project/$projectId/")({
  component: () => {
    const { projectId } = Route$g.useParams();
    return /* @__PURE__ */ jsx(
      Navigate,
      {
...
}), Route$f = createFileRoute("/_single/webcomponent/$")({
  component: WebcomponentPage
});
function WebcomponentPage() {
  const router = useRouter(), viewId = Route$f.useParams()._splat || "index", { colorScheme } = useMantineColorScheme();
...
function buildDiagramTreeData(views, groupBy) {
  const root2 = {
    ...
    for (; segments.length; ) {
      ...
      let node = r(parent.children, (n2) => n2.value === value);
...
function treeFromElements(elements) {
  const sorted = sortParentsFirst([...elements]), root2 = new Set(sorted), map2 = new Map(sorted.map((e2) => [e2.id, e2])), parents = new DefaultMap(() => null), children = sorted.reduce((acc, parent, index, all) => (acc.set(
    parent,
    all.slice(index + 1).filter(isDescendantOf(parent)).map((e2) => (root2.delete(e2), e2)).reduce((acc2, el) => (acc2.some(isAncestor(el)) || (acc2.push(el), parents.set(el, parent)), acc2), [])
  ), acc), new DefaultMap(() => []));
  return {
    sorted,
    byId: (id) => nonNullable(map2.get(id), `Element not found by id: ${id}`),
    root: root2,
...
function createNodes(prefix, elements, g) {
  ...
  for (const element of tree.sorted) {
    const isCompound = tree.children(element).length > 0, fqn = element.id, id = `${prefix}-${fqn}`, portId = isCompound ? `${id}${PortSuffix}` : id;
    graphNodes.set(fqn, {
      id,
      portId
    }), g.setNode(id, {
    ...
    parent && g.setParent(id, `${prefix}-${parent.id}`);
  }
  return {
    ...tree,
    byId: (id) => {
...
function applyDagreLayout(g) {
  return dagre.layout(g, {
    ...
    const node = g.node(nodeId), { x, y, width, height } = node;
    return {
      position: {
...
function useRelationshipsView(fqn) {
...
const Route$d = createFileRoute("/_single/relationships/$fqn")({
  component: ViewLayout$1
});
function ViewLayout$1() {
  const { fqn } = Route$d.useParams();
  return useRelationshipsView(fqn), /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsx(Box, { className: cssViewOutlet }) });
}
...
const Route$1 = createFileRoute("/project/$projectId/view/$viewId/")({
  ...
  component: ViewEditor
}), ProjectsRoute = Route$l.update({
  id: "/projects",
  path: "/projects",
  getParentRoute: () => Route$m
}), SingleRouteRoute = Route$k.update({
  id: "/_single",
  getParentRoute: () => Route$m
}), IndexRoute = Route$j.update({
  id: "/",
  path: "/",
  getParentRoute: () => Route$m
}), SingleSingleIndexRoute = Route$i.update({
  id: "/single-index",
  path: "/single-index",
  getParentRoute: () => SingleRouteRoute
}), ProjectProjectIdRouteRoute = Route$h.update({
  id: "/project/$projectId",
  path: "/project/$projectId",
  getParentRoute: () => Route$m
}), ProjectProjectIdIndexRoute = Route$g.update({
  id: "/",
  path: "/",
  getParentRoute: () => ProjectProjectIdRouteRoute
}), SingleWebcomponentSplatRoute = Route$f.update({
  id: "/webcomponent/$",
  path: "/webcomponent/$",
  getParentRoute: () => SingleRouteRoute
}), SingleViewViewIdRoute = Route$e.update({
  id: "/view/$viewId",
  path: "/view/$viewId",
  getParentRoute: () => SingleRouteRoute
}), SingleRelationshipsFqnRoute = Route$d.update({
  id: "/relationships/$fqn",
